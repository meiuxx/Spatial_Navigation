# perception/process.py  (updated – occupancy map integrated)
...
# Adjust resolution / extents to match your hospital floor plan scale.
# ---------------------------------------------------------------------------
occupancy_map = OccupancyMap(
    resolution  = 0.1,    # 10 cm per cell
    width_m     = 60.0,   # 60 m wide map
    height_m    = 60.0,   # 60 m deep map
    max_depth   = 15.0,   # discard depth > 15 m
    col_step    = 4,      # sample every 4th depth column  (tune for speed)
    row_step    = 4,      # sample every 4th depth row
    floor_band  = 1.2,    # only use points within ±1.2 m of camera height
)
...
def process_message(json_str, update_vis=False, vis_params=None):
    ...
    update occupancy map, and return detected frontiers.
    """
    try:
    ...
    # --- Update occupancy map (every frame, before saliency gate) -------------
    ...
    # Report frontier count so you can monitor map growth
    n_frontiers = occupancy_map.get_frontier_count()
    print(f"Occupancy map updated – {n_frontiers} frontiers detected")
    ...
    if sal_mean < SALIENCY_MEAN_THRESHOLD:
        print(f"Saliency too low ({sal_mean:.3f} < {SALIENCY_MEAN_THRESHOLD}) – skipping landmark")
        # We still updated the occupancy map above, so don't return early before that.
        occupancy_map.render(agent_pos=cam_pos, window_name="Occupancy Map")
        return

    # --- 3D position + bounding box -------------------------------------------
    result = get_object_3d_position(rgb_np, sal_map, depth_linear, depth_w, depth_h, fov)

    # Fix: get_object_3d_position returns (None, bbox) on invalid depth
    # and (pos, bbox, distance) on success.  Handle both safely.
    ...
    # Draw occupancy map with frontiers and agent position
    occupancy_map.render(agent_pos=cam_pos, window_name="Occupancy Map")

    cv2.waitKey(1)

    # --- World coordinate transformation & landmark storage -------------------
